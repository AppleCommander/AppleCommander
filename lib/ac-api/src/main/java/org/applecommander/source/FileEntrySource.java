...
public class FileEntrySource implements Source {
    ...
    public FileEntrySource(FileEntry fileEntry, Hint... hints) {
        ...
        this.buffer = fileEntry.getDataFork();
    ...
    public List<InformationGroup> information() {
        return InformationGroup.builder("FileEntry Source")
            .item("Name").value(fileEntry.getName())
            .item("Size").value(buffer.limit())
            .item("Hints").value(this.hints.stream().map(Hint::toString).collect(Collectors.joining(",")))
            .get();
    }
